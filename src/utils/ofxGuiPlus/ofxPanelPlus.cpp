...
ofImage ofxPanelPlus::loadIcon;
ofImage ofxPanelPlus::saveIcon;

ofxPanelPlus::ofxPanelPlus()
:bGrabbed(false){}

ofxPanelPlus::ofxPanelPlus(const ofParameterGroup & parameters, const std::string& filename, float x, float y)
: ofxGuiGroupPlus(parameters, filename, x, y)
, bGrabbed(false){
...
ofxPanelPlus * ofxPanelPlus::setup(const std::string& collectionName, const std::string& filename, float x, float y){
	if(!loadIcon.isAllocated() || !saveIcon.isAllocated()){
		loadIcons();
	}
	registerMouseEvents();
	return (ofxPanelPlus*)ofxGuiGroupPlus::setup(collectionName,filename,x,y);
}

ofxPanelPlus * ofxPanelPlus::setup(const ofParameterGroup & parameters, const std::string& filename, float x, float y){
	if(!loadIcon.isAllocated() || !saveIcon.isAllocated()){
		loadIcons();
	}
	registerMouseEvents();
	return (ofxPanelPlus*)ofxGuiGroupPlus::setup(parameters,filename,x,y);
}

void ofxPanelPlus::loadIcons(){
...
void ofxPanelPlus::generateDraw(){
	border.clear();
...
bool ofxPanelPlus::mouseReleased(ofMouseEventArgs & args){
    this->bGrabbed = false;
    if(ofxGuiGroupPlus::mouseReleased(args)) return true;
...
bool ofxPanelPlus::setValue(float mx, float my, bool bCheck){
